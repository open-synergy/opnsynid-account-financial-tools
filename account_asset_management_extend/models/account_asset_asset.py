...
class AccountAsset(models.Model):
    ...
    @api.multi
    @api.depends(
        "depreciation__line_ids",
        "depreciation__line_ids.line_date",
        "depreciation__line_ids.init_entry",
        "depreciation__line_ids.move_check",
        "depreciation__line_ids.type",
    )
    def _compute_last_posted_depreciation_line(self):
        ...
        for asset in self:
            ...
            if len(lines) > 0:
                _line_id = lines[0].id
            asset.last_posted_depreciation__line_id = _line_id
            ...
            if len(lines) > 0:
                _line_id = lines[0].id
            asset.last_posted_asset__line_id = _line_id
    ...
    def _compute_posted_depreciation__line_ids(self):
        obj_line = self.env["account.asset.depreciation.line"]
        for asset in self:
            domain = asset._prepare_posted_lines_domain()
            posted_lines = obj_line.search(domain, order="line_date desc")
            asset.posted_depreciation__line_ids = posted_lines.ids
    ...
    # TODO: Remove
    last_posted_depreciation__line_id = fields.Many2one(
    ...
    # TODO: Remove
    last_posted_asset__line_id = fields.Many2one(
        string="Last Asset Value Depreciation Line",
        comodel_name="account.asset.depreciation.line",
        compute="_compute_last_posted_depreciation_line",
    )
    # TODO: Remove
    posted_depreciation__line_ids = fields.Many2many(
        string="Posted Depreciation Lines",
        comodel_name="account.asset.depreciation.line",
        compute="_compute_posted_depreciation__line_ids",
    )
    ...
    def _prepare_posted_lines_domain(self):
        self.ensure_one()
        date = self.last_posted_asset__line_id.line_date
        return [
            "&",
            "&",
            "|",
    ...
    def _get_amount_to_depreciate(self):
        self.ensure_one()
        asset_value = self.last_posted_asset__line_id.remaining_value
    ...
    @api.multi
    def _create_depreciation_lines(self, table, _table_index_start, _line_index_start):
        self.ensure_one()
        _line_i_start = _line_index_start
        _table_i_start = _table_index_start
        posted_lines = self.posted_depreciation__line_ids
        obj_line = self.env["account.asset.depreciation.line"]
        seq = len(posted_lines)
        # SPONGE
        depr_line = self.last_posted_history_id
        # depr_line = self.last_posted_depreciation__line_id

        # last_date = table[-1]["lines"][-1]["date"]
        depreciated_value = sum(rec.amount for rec in posted_lines)

        for entry in table[_table_i_start:]:
            for line in entry["lines"][_line_i_start:]:
                seq += 1
    ...
    def _compute_starting_depreciation_entry(self, table):
        ...
        # TODO: Use new helper field
        last_line = self.last_posted_depreciation__line_id
        ...
        if len(posted_lines) > 0:
            ...
            if entry["date_stop"] == last_depreciation_date:
                ...
                if line["date"] == last_depreciation_date:
                    _line_i += 1
            _table_i_start = _table_i
            _line_i_start = _line_i
        else:  # no posted lines
            _table_i_start = 0
            _line_i_start = 0

        return _table_i_start, _line_i_start
