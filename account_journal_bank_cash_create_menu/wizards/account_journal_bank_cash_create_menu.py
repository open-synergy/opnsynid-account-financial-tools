...
class AccountJournalBankCashCreateMenu(models.TransientModel):
    ...
    parent_menu_id = fields.Many2one(string="Parent Menu", comodel_name="ir.ui.menu")

    sequence = fields.Integer(string="Sequence")

    search_view_id = fields.Many2one(
        string="Search View Ref", comodel_name="ir.ui.view"
    )
    ...
    def default_get(self, fields):
        res = super(AccountJournalBankCashCreateMenu, self).default_get(fields)
    ...
    def _create_window_action(self, journal):
        ...
        obj_act_window_view = self.env["ir.actions.act_window.view"]
    ...
    def button_create_menu(self):
        ...
        window_action_id = self._create_window_action(journal)

        self._create_menu(journal, window_action_id)
...
class AccountJournalBankCashCreateMenuViewDetail(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        string="Wizard", comodel_name="account.journal_bank_cash_create_menu"
    )

    sequence = fields.Integer(string="Sequence")

    view_id = fields.Many2one(string="Views", comodel_name="ir.ui.view")

    view_mode = fields.Selection(
        string="View Type", required=True, selection=VIEW_TYPES
    )
