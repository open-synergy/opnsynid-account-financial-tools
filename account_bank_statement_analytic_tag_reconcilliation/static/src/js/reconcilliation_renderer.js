...
*/
/* eslint complexity: "off", eqeqeq: "off", init-declarations: "off", no-use-before-define: "off" */
odoo.define(
    "account_bank_statement_analytic_tag_reconcilliation.ReconciliationRenderer",
    function (require) {
        "use strict";
        ...
        ReconciliationRenderer.LineRenderer.include({
            update: function (state) {
                ...
                _.each(state.reconciliation_proposition, function (prop) {
                    if (prop.display) {
                        props.push(prop);
                        if (prop.amount < 0) nb_debit_props += 1;
                        else if (prop.amount > 0) nb_credit_props += 1;
                    }
                });
                ...
                _.each(props, function (line) {
                    ...
                    if (
                        (line.already_paid === false &&
                            (state.balance.amount_currency < 0 ||
                                line.partial_reconcile) &&
                            nb_credit_props == 1 &&
                            line.amount > 0 &&
                            state.st_line.amount > 0 &&
                            state.st_line.amount < line.amount) ||
                        ((state.balance.amount_currency > 0 ||
                            line.partial_reconcile) &&
                            nb_debit_props == 1 &&
                            line.amount < 0 &&
                        ...
                        var $cell = $line.find(
                            line.amount > 0 ? ".cell_right" : ".cell_left"
                        );
                        var text;
                        if (line.partial_reconcile) {
                            ...
                            text = _t(
                                "This move's amount is higher than the transaction's amount. Click to register a partial payment and keep the payment balance open."
                            );
                        }

                        $(
                            '<span class="do_partial_reconcile_' +
                                !line.partial_reconcile +
                                ' line_info_button fa fa-exclamation-triangle"/>'
                        )
                            .prependTo($cell)
                ...
                this.$(".match .fa-chevron-right").toggleClass(
                    "disabled",
                    state.mv_lines.length <= state.limitMoveLines
                );
                this.$(".match .fa-chevron-left").toggleClass(
                    "disabled",
                    !state.offset
                );
                this.$(".match").css(
                    "max-height",
                    !state.mv_lines.length && !state.filter.length ? "0px" : ""
                );
                ...
                if (state.createForm) {
                    ...
                    this.model
                        .notifyChanges(this.handleCreateRecord, state.createForm)
                        .then(function () {
                            return self.model.notifyChanges(self.handleCreateRecord, {
                                analytic_tag_ids: {
                                    operation: "REPLACE_WITH",
                                    ids: _.pluck(
                                        state.createForm.analytic_tag_ids,
                                        "id"
                                    ),
                                },
                            });
